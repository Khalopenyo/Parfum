...
export default function SortingSearchBar({ onOpenFilters, total, favoritesCount }) {
  const { q, sort } = useCatalogFilters((state) => ({ q: state.q, sort: state.sort }));
  const { setSearch, setSort } = useCatalogFilters((state) => ({
